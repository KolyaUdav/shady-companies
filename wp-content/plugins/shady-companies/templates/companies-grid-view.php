        ...
        <?php foreach ($allPosts as $post): ?>
            <article class="company-card" style="border: 1px solid #eee; padding: 20px; background: #fff; border-radius: 8px;">
                <?php if (!empty($post['logo'])): ?>
                    <img src="<?php echo esc_url($post['logo']); ?>" alt="<?php echo $post['title']; ?>" style="max-height: 40px; margin-bottom: 10px; display: block;">
                <?php endif; ?>
                ...
                <?php if (!empty($post['cons'])): ?>
                    <div style="background: #fef2f2; border-left: 3px solid #ef4444; padding: 8px; margin-bottom: 10px; font-size: 0.9rem;">
                        <strong>Минусы:</strong> <?php echo esc_html($post['cons']); ?>
                    </div>
                <?php endif; ?>

                <div style="font-size: 0.9rem; color: #555;"><?php echo esc_html($post['excerpt']); ?></div>
            </article>
        <?php endforeach; ?>
    <?php else: ?>
